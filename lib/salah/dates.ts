 ...
 *
 * ⚠ MIRROR of functions/src/dates.ts — the function bodies must stay identical.
 * `__tests__/salahStreak.test.ts` compares them and fails the build on drift. The
 * duplication exists because the Cloud Functions package is deployed separately
 * and cannot import from the app (nor the app from it), and on the Spark plan the
 * streak engine runs client-side while functions/ stays dormant for a future
 * Blaze cutover. Both halves must agree on what "a day" means.
 *
